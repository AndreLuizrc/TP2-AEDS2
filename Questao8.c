...
void ShellSort(Personagem*, int, int*);

void IncercaoPorCor(Personagem*, int, int, int, int*);
...
int main(){
    ...
    ShellSort(selectedPersonagens, count_select, comp_mov);
...
void ShellSort(Personagem personagens[], int tam_vetor, int comp_mov[]){
    int h = 1;
    ...
    do{ 
        ...
        for(int cor = 0; cor < h; cor++){
            IncercaoPorCor(personagens, cor, h, tam_vetor, comp_mov);
        }
    }while(h > 1);
        
}

void IncercaoPorCor(Personagem personagens[], int cor, int h, int tam_vetor, int comp_mov[]){
    for(int i = (cor + h); i < tam_vetor; i += h){
...
void Log(int comparacoes, int movimentacoes, double tempoExecucao){
    ...
    if((log = fopen("824007_shellsort.txt","w")) != NULL){
